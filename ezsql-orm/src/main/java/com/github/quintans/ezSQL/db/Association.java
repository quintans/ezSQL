...
public class Association {
  ...
  // Many To Many
  public Association(String alias, Relationships relFrom, Relationships relTo) {
    Association fkFrom = new Association(relFrom.toArray(new Relation[0]));
    Association fkTo = new Association(relTo.toArray(new Relation[0]));
  ...
  public Association WITH(Column<?> column, Object value) {
    ...
    if (this.discriminatorTable != null && !this.discriminatorTable.equals(column.getTable()))
      throw new OrmException("Discriminator columns must belong to the same table. %s does not belong to %s.", column, this.discriminatorTable);
  ...
  private void defineAssociation(boolean add2Table, String alias, Relation... relations) {
    ...
    for (Relation relation : relations) {
      if (!tableFrom.equals(relation.getFrom().getColumn().getTable()))
        throw new OrmException("left side of %s does not belong to %s.", relation, tableFrom);
      else if (!tableTo.equals(relation.getTo().getColumn().getTable()))
        throw new OrmException("right side of %s does not belong to %s.", relation, tableTo);
    }

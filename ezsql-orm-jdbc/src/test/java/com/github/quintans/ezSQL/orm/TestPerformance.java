  ...
  public void testInsertDirectOrByReflectian() {
    tm.transactionNoResult(db -> {
      ...
      insert = db.insert(TEmployee.T_EMPLOYEE)
          ...
          .set(TEmployee.C_ID, 0L); // Force the ID placeholder generation for Postgresql. The Postgresql Translator does not generate ? for null IDs.
      SimpleJdbc jdbc = new SimpleJdbc(db.getJdbcSession());
      JdbcExecutor executor = new JdbcExecutor(translator, driver, jdbc);
  ...
  public void testQueryByReflectianAndByTransformer() {
    tm.transactionNoResult(db -> {
      ...
      query.list(r -> {
        ...
        dto.setSex(r.get(3, Boolean.class));
        dto.setCreation(r.get(4, Date.class));
        return dto;
      });
